...
//    datalist.append(TableColData(QStringLiteral("名称"), STK_DISPLAY_SORT_TYPE_NAME));
    ...
    datalist.append(TableColData(QStringLiteral("3日变动"), STK_DISPLAY_SORT_TYPE_LAST3, false));
    datalist.append(TableColData(QStringLiteral("5日变动"), STK_DISPLAY_SORT_TYPE_LAST5, false));
    datalist.append(TableColData(QStringLiteral("10日变动"), STK_DISPLAY_SORT_TYPE_LAST10, false));
...
void QShareTablewidget::setDataList(const ShareDataList &list)
{
    ...
    foreach (ShareData data, list) {
        ...
        this->setItemText(i, k++, QString("").sprintf("%.2f",data.mHistory.mLastMonthChgPers - 1));
        this->setItemText(i, k++, QString("").sprintf("%.2f",data.mHistory.mChgPersFromYear - 1));
